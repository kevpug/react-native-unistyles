...
export const useStyles = <ST extends StyleSheetWithSuperPowers>(
    ...
    const parsedStyles = useMemo(
        ...
        [theme, stylesheet, layout]
    );

    const dynamicStyleSheet = useMemo(
        () =>
            Object.entries(parsedStyles || {}).reduce((acc, [key, value]) => {
                ...
                return StyleSheet.create({
                    ...
                    [key]: withPlugins(
                        key,
                        parseStyle(value, variants, !isWeb || !unistyles.registry.config.experimentalCSSMediaQueries)
                    ),
                });
            }, {}),
